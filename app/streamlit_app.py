...
with left:
    st.subheader("1. Decide")
    persona = st.selectbox("Applicant persona", list_personas())
    col_a, col_b = st.columns(2)
    model_version = col_a.radio("Model", ["v1", "v2"], horizontal=True)
    contract_version = col_b.radio("Reason contract", [1, 2], horizontal=True, index=1)

    if st.button("Run decision", type="primary"):
        # Guard against Streamlit rerun semantics: decisions happen only
        # on explicit click, one snapshot per click.
    ...
    if record:
        ...
        if record.decision == "decline":
            ...
            for c in record.reason_codes:
                st.markdown(f"{c.rank}. **{c.code_id}** ({c.source}): {c.consumer_text}")
...
with right:
    st.subheader("2. Dispute (the point of the project)")
    st.caption(
    ...
    if st.button("Replay original decision"):
        try:
            replayed = st.session_state["replayed"] = store.replay(chosen)
    ...
    if replayed and replayed.decision_id == chosen:
        ...
        for c in replayed.reason_codes:
            st.markdown(f"{c.rank}. **{c.code_id}**: {c.consumer_text}")
